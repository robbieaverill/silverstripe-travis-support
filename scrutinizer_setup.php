 ...
 */
$requiredEnvs = array('SCRUTINIZER_SHA1', 'CORE_RELEASE');
...
if(!getenv('TRAVIS_TAG') && !getenv('SCRUTINIZER_BRANCH')) {
	echo("One of SCRUTINIZER_BRANCH or TRAVIS_TAG must be defined\n");
	exit(1);
}
...
$moduleVersion = getenv('TRAVIS_TAG') ?: getenv('SCRUTINIZER_BRANCH');
 ...
 * 7. Run it
 */
run("cd $modulePath; tar -cf $moduleArchivePath * .??*");
