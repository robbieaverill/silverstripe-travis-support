...
/**
 * Initialises a test project that can be built by CircleCI.
 *
 * Circle downloads the module, but in order to run unit tests it needs
 ...
 */
$requiredEnvs = array('CIRCLE_SHA1', 'CORE_RELEASE');
...
if (!getenv('CIRCLE_TAG') && !getenv('CIRCLE_BRANCH')) {
    echo("One of CIRCLE_BRANCH or CIRCLE_TAG must be defined\n");
    exit(1);
}
...
$moduleVersion = getenv('CIRCLE_TAG') ?: getenv('CIRCLE_BRANCH');
$moduleRef = getenv('CIRCLE_TAG')
